...
def parse_api_error(status_code: int, error_text: str) -> APIError:
    ...
    if status_code in [401, 403] or "authentication" in error_message_lower or "unauthorized" in error_message_lower:
...
def enhanced_on_backoff(details):
    ...
    if isinstance(exception, APIError):
        ...
        print(f"Retrying after {details['wait']:.1f}s (attempt {details['tries']}) - "
